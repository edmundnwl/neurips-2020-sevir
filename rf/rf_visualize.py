...
rf_visualize.py
----------------
Visual-diagnostics for the SEVIR-RF model.
• Confusion-matrix heat-map
• ROC curve with AUC
• 2-D PCA scatter (decision regions + test points)

Requires the model (`rf_model.joblib`) and, if you used PCA,
the transformer (`pca.joblib`) produced by rf_train.py.
"""
...
disp.ax_.set_title("Confusion matrix – SEVIR RF")
plt.tight_layout()
...
if pca is not None and pca.n_components_ >= 2:
    ...
    plt.tight_layout()
    plt.savefig("rf_pca_decision.png", dpi=300)
    plt.close()

print("[DONE] Visuals saved:\n"
      "  • rf_confusion_matrix.png\n"
      "  • rf_roc_curve.png    (if binary)\n"
      "  • rf_pca_decision.png (if PCA≥2)")