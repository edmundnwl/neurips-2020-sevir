...
def main():
    ...
    model = ConvGRUNowcast(input_dim=13, hidden_dim=[64, 128], output_dim=12).to(device)
    ...
    for epoch in range(5):
        ...
        for x, y in tqdm(train_loader, desc=f"Epoch {epoch+1} [Train]"):
            print(f"Input shape: {x.shape}, Target shape: {y.shape}")  # 调试信息
    ...
    torch.save(model.state_dict(), 'convgru_nowcast.pth')
